...
将 Robert C. Martin 的 [*Clean Code*](https://www.amazon.com/Clean-Code-Handbook-Software-Craftsmanship/dp/0132350882) 的代码原则应用于 JavaScript 。本文不是代码风格指南，而是一个用 JavaScript 生产 [可读，可复用，以及可重构的软件](https://www.notion.so/3rs-of-software-architecture.md) [（原文）](https://github.com/ryanmcdermott/3rs-of-software-architecture)的指南。

本文所涉及的原则不是人人适用，没必要条条遵循，通用的原则更是没几条。你可以把这些原则看作写代码时的参照，但它们是 *Clean Code* 作者多年的经验之谈。

我们软件行业只发展了短短半个世纪，还有很多需要我们去学习。等软件架构和建筑架构一样古老时，也许我们会有更多硬性规则要遵守。但现在，暂且让本文所涉及的这些原则成为你和你团队生产 JavaScript 代码的质量准则吧。

再提一句： 熟悉这些原则并不能立马让你成为一个更出色的软件开发，遵循这些原则多年也不意味着你不再犯错。每一段代码最初都是草稿，就像陶瓷最开始也是湿哒哒的粘土。当我们和同伴一起审代码、提高质量时，不要因为最初要改进的草稿代码而自责，要怪就怪这些代码！
...
我们要读的代码比要写的多得多，所以我们代码的可读性和可搜索性非常重要。使用无意义的变量名会让我们的程序难以理解，读者也读不懂，所以尽量使用可搜索的变量名。类似
...
默认参数往往比短路写法更清晰。注意，如果你使用短路，你的函数只会给未定义表达式赋予默认值。其他“不太正确”的值，如`''` 、`""`、`false`、`null`、`0`和`NaN`，均不会被替换为默认值。
...
限制函数的参数非常重要，因为它会让你的函数更容易测试。参数一旦超过三个就会难度大增，进而导致你不得不针对每个参数写大量测试用例。

没有参数最为理想，一个或两个也可以接受，三个就应避免，三个以上就应考虑重构。通常，如果你有一个大于两个参数的函数，那就意味着你要求这个函数做了太多事。否则，一般一个更高级的对象就能满足需求。
...
僵尸代码和冗余代码一样糟糕。没有理由在代码库中保存它。如果不会被调用，那就删掉。你需要它时，它依然保存在版本历史记录中。
